...
- Describe 3-4 different card designs vividly (floral border, soft pastel, traditional motif, modern minimal) since you can't actually show images
...
- Create urgency naturally: "slot কমে যাচ্ছে", "এই week-এ অনেক order আসছে"
- Be warm and sisterly/brotherly in tone
- If customer seems to have low budget → push Affordable
- If customer wants elegant/beautiful → push Premium`;
...
function TypingIndicator() {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "10px 14px", background: "white", borderRadius: 18, borderBottomLeftRadius: 4, width: "fit-content", boxShadow: "0 1px 4px rgba(0,0,0,0.08)" }}>
      {[0, 1, 2].map(i => (
        <div key={i} style={{
          width: 8, height: 8, borderRadius: "50%", background: BRAND.sky,
...
function MessageBubble({ msg, isNew }) {
  ...
  return (
    <div style={{
      ...
      {isAgent && (
        <div style={{
          width: 34, height: 34, borderRadius: "50%", background: `linear-gradient(135deg, ${BRAND.sky}, ${BRAND.navy})`,
          display: "flex", alignItems: "center", justifyContent: "center",
          fontSize: 14, marginRight: 8, flexShrink: 0, alignSelf: "flex-end",
          boxShadow: "0 2px 8px rgba(41,171,226,0.3)",
        }}>
          🌸
        </div>
      )}
      <div style={{
        ...
        background: isAgent ? "white" : `linear-gradient(135deg, ${BRAND.sky}, #1a8fc0)`,
        color: isAgent ? BRAND.navy : "white",
        fontSize: 14.5,
        lineHeight: 1.6,
        boxShadow: isAgent ? "0 1px 4px rgba(0,0,0,0.08)" : "0 2px 8px rgba(41,171,226,0.35)",
        whiteSpace: "pre-wrap",
...
export default function BoondhonSalesAgent() {
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      const newMessages = [...updated, { role: "assistant", content: "দুঃখিত, একটু সমস্যা হচ্ছে। একটু পরে আবার try করুন 😊" }];
      setMessages(newMessages);
    } finally {
      setLoading(false);
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: `linear-gradient(160deg, ${BRAND.navy} 0%, #0d2040 100%)`,
      display: "flex",
      alignItems: "center",
      ...
      <div style={{
        ...
        <div style={{
          background: `linear-gradient(135deg, ${BRAND.navy} 0%, #0d2a50 100%)`,
          padding: "16px 20px",
          display: "flex",
          alignItems: "center",
          gap: 12,
          borderBottom: `1px solid rgba(41,171,226,0.2)`,
        }}>
          <div style={{
            width: 46, height: 46, borderRadius: "50%",
            background: `linear-gradient(135deg, ${BRAND.sky} 0%, #1a8fc0 100%)`,
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 20, boxShadow: `0 0 0 3px rgba(41,171,226,0.25)`,
            flexShrink: 0,
          }}>🌸</div>
          <div style={{ flex: 1 }}>
            <div style={{ color: "white", fontWeight: 700, fontSize: 15.5, letterSpacing: 0.2 }}>
              BOONDHON Printing House
            </div>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ color: BRAND.gold, fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase" }}>Est. 2019</div>
            <div style={{ color: "#4a6a8a", fontSize: 10.5, marginTop: 2 }}>Manikganj, BD</div>
          </div>
        </div>

        <div style={{
          flex: 1, overflowY: "auto", background: BRAND.chatBg,
          padding: "16px 14px 8px",
          backgroundImage: `radial-gradient(circle at 20% 80%, rgba(41,171,226,0.04) 0%, transparent 50%), radial-gradient(circle at 80% 20%, rgba(201,168,76,0.04) 0%, transparent 50%)`,
        }}>
          {messages.map((msg, i) => (
        ...
        {quickReplies.length > 0 && (
          <div style={{
            background: BRAND.chatBg, padding: "6px 14px 10px",
        ...
        <div style={{
          ...
          <textarea
            ...
            rows={1}
            style={{
              flex: 1, fontSize: 14.5, color: BRAND.navy,
              fontFamily: "'Hind Siliguri', 'Noto Sans Bengali', sans-serif",
              lineHeight: 1.5, maxHeight: 80, overflowY: "auto", padding: "6px 2px",
            }}
          />
          <button
            ...
            style={{
              ...
              background: input.trim() && !loading ? `linear-gradient(135deg, ${BRAND.sky}, #1a8fc0)` : "#dde4ed",
              color: "white", fontSize: 16,
              cursor: input.trim() && !loading ? "pointer" : "default",
              display: "flex", alignItems: "center", justifyContent: "center",
              transition: "all 0.2s", flexShrink: 0,
              boxShadow: input.trim() && !loading ? "0 3px 10px rgba(41,171,226,0.4)" : "none",
            }}
          >➤</button>
        </div>
